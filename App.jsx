...
export default function App() {
  ...
  function submitMc(optionIdx) {
    ...
    setMcAnswers((a) => [...a, { correct, question: q.q, options: q.options, selectedIndex: optionIdx, correctIndex: q.correct }]);
    setStep((s) => s + 1);
  }
  ...
  async function finishQuiz(finalOpen) {
    ...
    const result = {
      ...
      mcCorrect: 5 - mcWrong,
      mcAnswers: mcAnswers,
      openCorrect: graded.filter((g) => g.correct).length,
      openGraded: graded,
          ...
          <div className="pop-in">
            <div style={{ ...glass, padding: 32, textAlign: "center", marginBottom: 16 }}>
              ...
              <div style={{ color: textMuted, marginTop: 6 }}>
                {LEVEL_META[lastResult.level].title} · {lastResult.mcCorrect}/5 с вариантами верно · {lastResult.openCorrect}/5 открытых верно
              </div>
            ...
            <div style={{ ...glass, padding: 20, marginBottom: 16 }}>
              <div className="display-font" style={{ fontWeight: 700, marginBottom: 12 }}>Разбор вопросов с вариантами ответа</div>
              {(lastResult.mcAnswers || []).map((m, i) => (
                <div key={i} style={{ padding: "10px 0", borderTop: i ? `1px solid ${cardBorder}` : "none" }}>
                  <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                    {m.correct ? <Check size={16} color="#00E5A0" style={{ marginTop: 3, flexShrink: 0 }} /> : <X size={16} color="#FF5D6C" style={{ marginTop: 3, flexShrink: 0 }} />}
                    <div style={{ fontSize: 13, fontWeight: 700 }}>{m.question}</div>
                  </div>
                  <div style={{ fontSize: 12, color: textMuted, marginLeft: 24, marginTop: 4 }}>
                    Ваш ответ: {m.options[m.selectedIndex]}
                  </div>
                  {!m.correct && (
                    <div style={{ fontSize: 12, color: "#00E5A0", marginLeft: 24, marginTop: 2 }}>
                      Правильно: {m.options[m.correctIndex]}
                    </div>
                  )}
                </div>
              ))}
            ...
            <div style={{ ...glass, padding: 20, marginBottom: 16 }}>
              <div className="display-font" style={{ fontWeight: 700, marginBottom: 12 }}>Разбор открытых вопросов</div>
              {lastResult.openGraded.map((g, i) => (
                <div key={i} style={{ padding: "10px 0", borderTop: i ? `1px solid ${cardBorder}` : "none" }}>
                  <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                    {g.correct ? <Check size={16} color="#00E5A0" style={{ marginTop: 3, flexShrink: 0 }} /> : <X size={16} color="#FF5D6C" style={{ marginTop: 3, flexShrink: 0 }} />}
                    <div style={{ fontSize: 13, fontWeight: 700 }}>{g.question}</div>
                  </div>
                  <div style={{ fontSize: 12, color: textMuted, marginLeft: 24, marginTop: 4 }}>
                    Ваш ответ: {g.text ? g.text : "— (не написан)"}
                  </div>
                  {!g.correct && (
                    <div style={{ fontSize: 12, color: "#00E5A0", marginLeft: 24, marginTop: 2 }}>
                      Ожидались темы: {g.keywords.join(", ")}
                    </div>
                  )}
                  <div style={{ fontSize: 12, color: textMuted, marginLeft: 24, marginTop: 2, fontStyle: "italic" }}>{g.comment}</div>
                </div>
              ))}
            </div>
